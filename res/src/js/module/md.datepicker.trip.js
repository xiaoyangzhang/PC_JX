define(function(require, exports, module) {
    ...
    $datepicker.prototype = {
                    ...
                    $('.datepicker .price').each(function(index){
                        ...
                        if(isSetData){
                                        ...
                                        $.each(months, function(index, month) {
                                            $.each(month.days, function(index, day) {
                                                ...
                                                $('.datepicker .day .choiced').each(function(){
                                                    var cur_time = new Date($('#SY').text(), $('.tdmonth li.on').index(), $(this).find('font').text()).getTime() + '';
                                                    if(cur_time == time){
                                                        skuIdArr = $(this).attr('data-sku-id') && JSON.parse($(this).attr('data-sku-id'));
                                                        switch(pTxt){
                                                            case '成' :
                                                                skuId = Number(skuIdArr[0]);
                                                                break;
                                                            case '儿' :
                                                                skuId = Number(skuIdArr[1]);
                                                                break;
                                                            case '单房差' :
                                                                skuId = Number(skuIdArr[2]);
                                                                break;
                                                        }
                                                    }
                                                });
                            ...
                            if($.inArray(cur_month,_self.months) > -1){
                                $dtbx.each(function(){
                                    _self.update(price.val(), stock.val(), name, pTxt, skuId, $(this).find('font').text());
                                });
        ...
        set_tdvalue: function(obj, price, stock, pTxt, skuId) {
            ...
            if (obj.find('.tipvl').length == 0){
                if(skuId){
                    ...
                    obj.append('<div class="tipvl"><label>'+pTxt+'￥</label><label class="price_" data-sku-id="'+ skuId +'" data-pTxt="'+pTxt+'">' + price + '</label><br><label>库</label><label class="stock_" data-sku-id="'+ skuId +'">' + stock + '</label></div>');
                }else{
                    obj.append('<div class="tipvl"><label>'+pTxt+'￥</label><label class="price_" data-pTxt="'+pTxt+'">' + price + '</label><br><label>库</label><label class="stock_">' + stock + '</label></div>');
                }
                
            }else{
                if(skuId){
                    html += '<br><label>'+pTxt+'￥</label><label class="price_" data-sku-id="'+ skuId +'" data-pTxt="'+pTxt+'">' + price + '</label>';
                    if(stock){
                        html += '<br><label>库</label><label class="stock_" data-sku-id="'+ skuId +'" data-pTxt="'+pTxt+'">' + stock + '</label>';
                    }
                }else{
                    html += '<br><label>'+pTxt+'￥</label><label class="price_" data-pTxt="'+pTxt+'">' + price + '</label>';
                    if(stock){
                        html += '<br><label>库</label><label class="stock_" data-pTxt="'+pTxt+'">' + stock + '</label>';
                    }
                }
        ...
        update: function(price, stock, name, pTxt, skuId, day) {
        ...
        updateSkuId: function(obj,price,stock,pTxt){
            ...
            $(obj).find('.price_').each(function(){
                ...
                if(pTxt == $(this).attr('data-pTxt')){
                    skuId = $(this).attr('data-sku-id'),
                    oldPrice = $(this).text(),
                    oldStock = $(this).next().next().next('.stock_').text();
                    if(oldPrice != price){
