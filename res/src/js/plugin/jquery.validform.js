(function ($, win, undef) {
	...
	var tipmsg = {
		...
		w : {
			...
			"s" : "不能输入特殊字符！",
			"s6-18" : "请填写6到18位字符！",
			"pwd" : "请输入6-16位密码，需同时包含字母及数字！",
			"p" : "请填写邮政编码！",
			"m" : "请填写手机号码！",
			"tel" : "请填写座机例:(0731-83187200)或手机号码!",
			"e" : "邮箱地址格式不对！",
			"url" : "请填写网址！",
			"card" : "请填写正确（身份证号）例如：462245199003044859！",
			"dlic" : "请填写正确（驾驶证号）例如：450401196501！",
			"psport" : "请填写正确（护照号）例如：G-15525453！",
			"gidcard" : "请填写正确（导游证号）例如：D-4621-005480！",
			"null_n" : "请填写数字！",
			"null_n6-16" : "请填写6到16位数字！",
			"null_s" : "不能输入特殊字符！",
			"null_s6-18" : "请填写6到18位字符！",
			"null_p" : "请填写邮政编码！",
			"null_m" : "请填写手机号码！",
			"null_tel" : "请填写座机例:(0731-83187200)或手机号码!",
			"null_e" : "邮箱地址格式不对！",
			"null_url" : "请填写网址！"
		},
		def : "请填写正确信息！",
...
//		r : "输入正确",
	...
	var Validform = function (forms, settings, inited) {
		...
		forms.each(function () {
			...
			$this.delegate("[datatype]", "blur", function () {
				...
				var self = this;
				setTimeout(function(){Validform.util.check.call(self,$this, subpost)},100);
			});
			$this.delegate(":text", "keypress", function (event) {
				if (event.keyCode == 13 && $this.find(":submit").length == 0) {
					setTimeout(function(){$this.submit()},50);
				}
			});
			Validform.util.enhance.call($this, curform.settings.tiptype, curform.settings.usePlugin, curform.settings.tipSweep);
			curform.settings.btnSubmit && $this.find(curform.settings.btnSubmit).bind("click", function () {
				setTimeout(function(){$this.trigger("submit")},50);
	...
	Validform.util = {
		dataType : {
			...
			"n6-16" : /^\d{6,16}$/,
			"pwd" : /^(?=.{6,16}$)(?![0-9]+$)(?![a-zA-Z]+$)[0-9a-zA-Z]+$/,
			"n" : /^\d+$/,
			"s" : /^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]+$/,
			"p" : /^[0-9]{6}$/,
			"m" : /^1[0-9]{10}$/,
			"tel" : /^0\d{2,3}-?\d{7,8}$|^1[0-9]{10}$/,
			"e" : /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
			"url" : /^(\w+:\/\/)?\w+(\.\w+)+.*$/,
			"card" : /^(\d{15}$|^\d{16}$|^\d{17}$|^\d{18}$|^\d{17}[a-zA-z])$/,
			"dlic" : /^\d{12}$/,
			"psport" : /^[a-zA-z]-?\d{7,8}$/,
			"gidcard" : /^[a-zA-z]-?\d{4}-?\d{6}$/,
			"null_s6-18" : /^([\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]{6,18})?$/,
			"null_n6-16" : /^(\d{6,16})?$/,
			"null_n" : /^(\d+)?$/,
			"null_s" : /^([\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]+)?$/,
			"null_p" : /^([0-9]{6})?$/,
			"null_m" : /^(1[0-9]{10})?$/,
			"null_tel" : /^(0\d{2,3}-?\d{7,8}|1[0-9]{10})?$/,
			"null_e" : /^(\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*)?$/,
			"null_url" : /^((\w+:\/\/)?\w+(\.\w+)+.*)?$/
		},
		...
		getValue : function (obj) {
			...
			if (obj.is(":radio")) {
				...
				inputval = obj.val()==obj.attr("placeholder")?obj.val():obj.val();
			}
		...
		enhance : function (tiptype, usePlugin, tipSweep, addRule) {
			...
			curform.find("[datatype]").each(function () {
				var $this = $(this).closest(".gf-select").length > 0 ? $(this).closest(".gf-select") : $(this);
				if (tiptype == 2) {
		...
		showmsg : function (msg, type, o, triggered) {
			...
			o.obj = o.obj.closest(".gf-select").length > 0 ? o.obj.closest(".gf-select") : o.obj;
			if (type == 2 && o.obj) {
		...
		cssctl : function (obj, status) {
			switch (status) {
			case 1:
				obj.removeClass("Validform_right Validform_wrong").addClass("Validform_checktip");
				break;
			case 2:
				obj.removeClass("Validform_wrong").addClass("Validform_checktip Validform_right");
				break;
			case 4:
				obj.removeClass("Validform_right Validform_wrong").addClass("Validform_checktip");
				break;
			default:
				obj.removeClass("Validform_right").addClass("Validform_checktip Validform_wrong");
			}
		},
		...
		resetForm : function () {
			...
			brothers.find(".Validform_checktip").removeClass("Validform_wrong Validform_right");
